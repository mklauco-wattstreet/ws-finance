...
* **Production commands MUST use both compose files:** Always use `-f docker-compose.yml -f docker-compose.prod.yml` for any `docker compose` command on production. Omitting `docker-compose.prod.yml` puts the container on the wrong network and breaks pgbouncer DNS resolution. Never suggest a bare `docker compose` command for production use.
