...
* **60-min tables:** Every grid source has a `<source>_60min` mirror table (migrations 057-063). See [docs/60min_tables_plan.md](docs/60min_tables_plan.md) for the column inventory, aggregation rules, and live-aggregator design (§9). The 7 aggregators are the existing `app/backfill/backfill_*_60min.py` scripts invoked with `--auto`; they fire at `15,30,45,0 * * * *` (1 min after the 15-min source runners) with a strict `HAVING COUNT(DISTINCT time_interval) = 4` completeness gate — partial hours produce no row.
