#!/usr/bin/env python3
"""
ENTSO-E Unified Scheduled Generation Runner.

Fetches day-ahead scheduled generation (A71) for ALL active areas,
parses the XML into records with area_id, and uploads
to the partitioned entsoe_generation_scheduled table.
...
Usage:
    python3 entsoe_unified_scheduled_runner.py [--debug] [--dry-run]
    python3 entsoe_unified_scheduled_runner.py --start 2024-12-01 --end 2024-12-22
"""
...
class UnifiedScheduledRunner(BaseRunner):
    """Unified runner for ENTSO-E Scheduled Generation data (A71) - All Areas."""

    RUNNER_NAME = "ENTSO-E Unified Scheduled Generation Runner"
    ...
    def _fetch_data_for_area(self, period_start, period_end, area_code: str) -> str:
        """Fetch scheduled generation XML for a specific area."""
        xml_content = self.client.fetch_scheduled_generation_for_domain(
    ...
    def _save_xml_file(
        ...
        output_file = self.get_output_path(
            f'entsoe_gen_scheduled_{country_code.lower()}_{start_str}_{end_str}.xml',
            period_start
        )
    ...
    def _parse_data(self, xml_file: Path, area_id: int, country_code: str) -> List[dict]:
        """Parse XML file into records with area_id and country_code."""
        parser = ScheduledGenerationParser(area_id=area_id, country_code=country_code)
    ...
    def run(self) -> bool:
        """Execute the unified scheduled generation data pipeline for all areas."""
        self.print_header()
