#!/usr/bin/env python3
"""
ENTSO-E Scheduled Generation Runner (A71).

Fetches day-ahead scheduled generation data,
parses the XML into records, and uploads to PostgreSQL
database using bulk upserts.

This script runs every 15 minutes via cron.

Wide-format columns:
- scheduled_total_mw: Total scheduled generation (aggregated)

Usage:
    python3 entsoe_gen_scheduled_runner.py [--debug] [--dry-run]
    python3 entsoe_gen_scheduled_runner.py --start 2024-12-01 --end 2024-12-15
"""
...
from entsoe.parsers import ScheduledGenerationParser


class ScheduledGenerationRunner(BaseRunner):
    """Runner for ENTSO-E Scheduled Generation data (A71)."""

    RUNNER_NAME = "ENTSO-E Scheduled Generation Runner"

    # Table configuration
    TABLE_NAME = "entsoe_generation_scheduled"
    COLUMNS = [
    ...
    def _fetch_data(self, period_start, period_end) -> str:
        """
        Fetch scheduled generation XML.
        ...
        """
        self.logger.info("")
        self.logger.info("Fetching Scheduled Generation (A71)...")
        xml_content = self.client.fetch_scheduled_generation(period_start, period_end)
    ...
    def _save_xml_file(
        ...
        output_file = self.get_output_path(
            f'entsoe_gen_scheduled_{start_str}_{end_str}.xml',
            period_start
        )
    ...
    def _parse_data(self, xml_file: Path) -> List[dict]:
        """Parse XML file into records."""
        self.logger.info("")
        self.logger.info("Parsing XML data...")

        self.parser = ScheduledGenerationParser()
        ...
        if data and self.debug:
            ...
            value = sample.get('scheduled_total_mw')
            value_str = f"{value:.1f} MW" if value is not None else "N/A"
            self.logger.debug(f"    scheduled_total_mw: {value_str}")
    ...
    def run(self) -> bool:
        """Execute the scheduled generation data pipeline."""
        self.print_header()
...
if __name__ == '__main__':
    ScheduledGenerationRunner.main()
