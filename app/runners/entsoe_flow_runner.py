#!/usr/bin/env python3
"""
ENTSO-E Cross-Border Physical Flows Runner (Wide Format).

Fetches physical flows (A11) data for all CZ border pairs,
...
Usage:
    python3 entsoe_flow_runner.py [--debug] [--dry-run]
"""
...
class FlowRunner(BaseRunner):
    """Runner for ENTSO-E Cross-Border Physical Flows data (A11) - Wide Format."""
    ...
    def _fetch_all_borders(self, period_start, period_end) -> Dict[str, str]:
        """
        Fetch A11 XML for all CZ border pairs.
        ...
        for neighbor_key, neighbor_eic in CZ_NEIGHBORS.items():
            self.logger.info("")
            self.logger.info(f"Fetching flows for CZ <-> {neighbor_key.upper()}...")
    ...
    def _save_xml_files(
        self, xml_data: Dict[str, str], period_start, period_end
    ) -> Dict[str, Path]:
        """
        Save all XML files to disk.

        Args:
            xml_data: Dict mapping border key to XML content
            period_start: Period start datetime
            period_end: Period end datetime

        Returns:
            Dict mapping border key to saved file path
        """
        saved_files = {}
        ...
        for border_key, xml_content in xml_data.items():
            output_file = self.get_output_path(
                f'entsoe_flow_{border_key}_{start_str}_{end_str}.xml',
                period_start
            )
    ...
    def _parse_data(self, xml_files: Dict[str, Path]) -> List[dict]:
        ...
        if data and self.debug:
            sample = data[0]
            self.logger.debug(f"  Sample record for {sample['delivery_datetime']}:")
            for col in CrossBorderFlowsParser.FLOW_COLUMNS:
                value = sample.get(col)
                value_str = f"{value:.1f} MW" if value is not None else "N/A"
                self.logger.debug(f"    {col}: {value_str}")
            total = sample.get('flow_total_net_mw')
            total_str = f"{total:.1f} MW" if total is not None else "N/A"
            self.logger.debug(f"    flow_total_net_mw: {total_str}")
