#!/usr/bin/env python3
"""
Download imbalance price reports from OTE-CR website.

Usage:
    python3 download_imbalance_prices.py [START_DATE END_DATE] [--debug]

Examples:
    # Auto mode - downloads missing files from last download to yesterday
    python3 download_imbalance_prices.py
    python3 download_imbalance_prices.py --debug

    # Manual mode - specify date range
    python3 download_imbalance_prices.py 2025-10-15 2025-10-31
    python3 download_imbalance_prices.py 2025-10-15 2025-10-31 --debug
...
def build_download_url(date):
    """
    Build the OTE-CR download URL for imbalance prices.

    URL pattern: https://www.ote-cr.cz/pubweb/attachments/05_09_12/{YYYY}/month{MM}/day{DD}/Imbalances_{DD}_{MM}_{YYYY}_V0_EN.xlsx
    ...
    base_url = "https://www.ote-cr.cz/pubweb/attachments/05_09_12"
    url_path = f"{year}/month{month}/day{day}"
    filename = f"Imbalances_{day}_{month}_{year}_V0_EN.xlsx"
...
def download_report(date, base_dir, logger):
    """
    Download imbalance report for a specific date.

    Args:
        date: datetime object
        base_dir: Base directory for organizing files (prices_imbalance)
...
def main():
    ...
    if not auto_mode and not manual_mode:
        print("Usage: python3 download_imbalance_prices.py [START_DATE END_DATE] [--debug]")
        print("\nExamples:")
        print("  # Auto mode - downloads missing files")
        print("  python3 download_imbalance_prices.py")
        print("  python3 download_imbalance_prices.py --debug")
        print("\n  # Manual mode - specify date range")
        print("  python3 download_imbalance_prices.py 2025-10-15 2025-10-31")
        print("  python3 download_imbalance_prices.py 2025-10-15 2025-10-31 --debug")
        print("\nDates should be in YYYY-MM-DD format")
        sys.exit(1)
    ...
    # Get the script's directory (prices_imbalance)
    ...
    if auto_mode:
        ...
        if start_date is None or end_date is None:
            ...
            run_upload_script(
                upload_script_name='upload_imbalance_prices.py',
                base_dir=script_dir,
        ...
        print_banner("OTE-CR Imbalance Price Report Downloader (MANUAL)", debug_mode)
