#!/usr/bin/env python3
"""
Download day-ahead price reports from OTE-CR website.

Usage:
    python3 download_day_ahead_prices.py [START_DATE END_DATE] [--debug]
...
def build_download_url(date):
    ...
    Note: OTE-CR changed their URL structure on October 1, 2025:
    - Before Oct 1, 2025: https://www.ote-cr.cz/pubweb/attachments/01/{YYYY}/month{MM}/day{DD}/DM_{DD}_{MM}_{YYYY}_EN.xlsx
    - From Oct 1, 2025: https://www.ote-cr.cz/attachments/01/{YYYY}/month{MM}/day{DD}/DM_15MIN_{DD}_{MM}_{YYYY}_EN.xlsx
    ...
    # Check if date is before or after October 1, 2025
    cutoff_date = datetime(2025, 10, 1)

    url_path = f"{year}/month{month}/day{day}"

    if date < cutoff_date:
        # Old format (before Oct 1, 2025) - uses /pubweb/attachments
        base_url = "https://www.ote-cr.cz/pubweb/attachments/01"
        filename = f"DM_{day}_{month}_{year}_EN.xlsx"
    else:
        # New format (from Oct 1, 2025) - uses /attachments and includes "15MIN" in filename
        base_url = "https://www.ote-cr.cz/attachments/01"
        filename = f"DM_15MIN_{day}_{month}_{year}_EN.xlsx"
...
def get_filename(date):
    ...
    cutoff_date = datetime(2025, 10, 1)
    if date < cutoff_date:
        return f"DM_{date.strftime('%d_%m_%Y')}_EN.xlsx"
    else:
        return f"DM_15MIN_{date.strftime('%d_%m_%Y')}_EN.xlsx"


def download_report(date, base_dir, logger):
    """
    Download day-ahead report for a specific date.
...
def main():
    ...
    if not auto_mode and not manual_mode:
        print("Usage: python3 download_day_ahead_prices.py [START_DATE END_DATE] [--debug]")
        print("\nExamples:")
        print("  # Auto mode - downloads missing files")
        print("  python3 download_day_ahead_prices.py")
        print("  python3 download_day_ahead_prices.py --debug")
        print("\n  # Manual mode - specify date range")
        print("  python3 download_day_ahead_prices.py 2025-05-01 2025-05-31")
        print("  python3 download_day_ahead_prices.py 2025-05-01 2025-05-31 --debug")
        print("\nDates should be in YYYY-MM-DD format")
        sys.exit(1)
    ...
    if auto_mode:
        ...
        print_banner("OTE-CR Day-Ahead Price Report Downloader (MANUAL)", debug_mode)
    ...
    try:
        ...
        for i, date in enumerate(dates, 1):
            ...
            # Wait random time between 1-3 seconds between downloads (except for the last one)
            if i < len(dates):
